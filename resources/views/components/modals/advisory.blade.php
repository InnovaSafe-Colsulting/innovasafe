...
<div id="modal-asesoria" class="fixed inset-0 z-[100] hidden items-center justify-center">
    ...
    <div class="relative w-full max-w-md mx-4 bg-[#01020e] rounded-2xl p-8 border border-white/10 max-h-[90vh] overflow-y-auto">
        ...
        <form id="form-asesoria" class="space-y-4">
            ...
            <div>
                ...
                <select name="type_service[]" multiple
                    class="w-full px-4 py-2 bg-white/5 border border-white/10 rounded-lg text-white focus:ring-2 focus:ring-blue-500 focus:border-transparent outline-none transition"
                    style="height:120px;">
                    @foreach($typeServices as $service)
                        <option value="{{ $service->id }}" class="py-1.5 px-2 hover:bg-blue-600 bg-[#0d0e1f]">{{ $service->name }}</option>
                    @endforeach
                </select>
                <p class="text-xs text-gray-500 mt-1">Mantén <kbd class="px-1 py-0.5 bg-white/10 rounded text-gray-300 text-xs">Ctrl</kbd> presionado para seleccionar varias opciones.</p>
